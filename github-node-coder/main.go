// A generated module for NodeCoder functions

package main

import (
	"context"
	"dagger/node-coder/internal/dagger"
	"fmt"
)
...
// Ask a node-coder to complete a task and get the Container with the completed task
func (m *NodeCoder) Assignment(
	// The task to complete
	task string,
) *dagger.Container {
	// Create a workspace for building node code
	ws := dag.Workspace(dagger.WorkspaceOpts{
		BaseImage: "node:20",
		Context:   dag.Directory(),
		Checker:   "npm run test && npm start",
	})

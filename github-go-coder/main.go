...
func createNewRepo(githubToken, newRepoName string) error {
    if newRepoName == "" {
        return nil
    }
...
func (m *GoCoder) SolveIssue(
    ...
    // +optional
    model string,
    // New repository name if a new project is to be created
    newRepoName string,
) (string, error) {
    ...
    // Create new repo if newRepoName is provided
    if err := createNewRepo(string(githubToken), newRepoName); err != nil {
